...
document
  .getElementById("character-picker")
  .addEventListener("click", (event) => {
    const card = event.target.closest(".character-card");
    const character = totalCharacters.find(
      (c) => c.name === card.querySelector("#name").textContent,
    );
    if (!card) return;
    ...
    document.getElementById("recent-choice").textContent =
      `You chose: ${character.name}`;
    document.getElementById("turns-number").textContent =
      `Round: ${numberofturns}`;
...
function displayCurrentData() {
  ...
  do {
    ...
    seenCharacters.includes(totalCharacters[randomchoice2]) ||
    randomchoice2 == randomchoice1
  );

  console.log(
    "totalcharactersoptions",
    totalCharacters[randomchoice1],
    totalCharacters[randomchoice2],
  );
  ...
  if (totalCharacters.length - seenCharacters.length == 1) {
    const remainingCharacter = totalCharacters.filter(
      (character) => !seenCharacters.includes(character),
    );
